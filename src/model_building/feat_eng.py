...
"""This script runs the feature engineering and extracts two features from the preprocessed data.


Usage: feat_eng.py --output_file_path=<output_file_path> --input_file_path=<input_file_path>
 
Options:
--output_file_path=<output_file_path>   Path to folder and file name where the file with new features will be saved, including the name, `in quotes.
--input_file_path=<input_file_path>		Path to folder and file name where the preprocessed file is stored, including the name, `in quotes.
"""
...
def main(output_file_path, input_file_path):
    ...
    preprocessed_df = preprocessed_df.assign(name_sent=preprocessed_df['name'].apply(get_sentiment))

    # Clean the inital columns
