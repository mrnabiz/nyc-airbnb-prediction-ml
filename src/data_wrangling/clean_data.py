...
This cleaning phase is all about removing unnecessary columns and prepare the 
data for EDA.
...
the cleaned data will be saved, including the name, `in quotes.
--input_file_path=<input_file_path>			Path to folder and file name where 
the raw file is stored, including the name, `in quotes.
"""
...
def main(output_file_path, input_file_path):
    ...
    drop_columns = ['scrape_id', 'last_scraped', 'source', 'picture_url', 
                    'host_thumbnail_url', 'host_neighbourhood', 
    ...
    # Drop Null ID rows
    clean_df = clean_df[pd.to_numeric(clean_df['id'], 
