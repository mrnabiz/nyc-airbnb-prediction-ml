...
"""This script runs the EDA analysis over the cleaned data and the preprocessed data.
...
Path to folder and file name where the clean df is stored, including the name, `in quotes.
...
def main(output_dir, clean_df_file_path, preprocessed_df_file_path):
    ...
    preprocessed_full_df = pd.read_csv(preprocessed_df_file_path, low_memory=False)

    # A random split of the data to dcrease the analysis time
    clean_df, clean_df_remain = train_test_split(clean_full_df, 
                                                 test_size=0.95, random_state=123)
    preprocessed_df, preprocessed_df_remain = train_test_split(preprocessed_full_df, 
                                                 test_size=0.95, random_state=123)
        ...
        clean_df.sort_values(
            ...
            ignore_index=True).isna().reset_index().melt(id_vars='index')).mark_rect().encode(
                ...
                alt.Stroke('value')).properties(
                    width=800,
                    title="The accurance of Null values in the dataset sorted based on the \
                        host joining date"
                    ).configure_title(
                        fontSize=20,
                        font='Cambria',
    ...
    numeric_cols_hist = alt.Chart(preprocessed_df).mark_bar().encode(
        ...
        alt.Y('count()', axis=alt.Axis(title='Count'), stack=False)).properties(
            width=250,
            height=150
    ...
    # Build the corrlation matrix
    corr_df = preprocessed_df.drop(['id'], axis=1).corr('spearman').style.background_gradient()

    # Correlation of the numeric feature with the correlation above 0.1
    numeric_corr_cols = ['host_listings_count', 'price', 'minimum_nights', 'number_of_reviews', 'reviews_per_month']

    numeric_cols_corr = alt.Chart(preprocessed_df).mark_circle(opacity=0.3).encode(
        alt.X(alt.repeat("column"), type='quantitative', scale=alt.Scale(zero=False)),
        alt.Y(alt.repeat("row"), type='quantitative', scale=alt.Scale(zero=False))).properties(
            width=150,
            height=150
            ).repeat(
                ...
                column=numeric_corr_cols
                    ...
                    title="Pair-wise correlation of numeric columns in AirBnb dataset").configure_title(
                        fontSize=20,
                        font='Cambria',
                        anchor='start'
        )
...
if __name__ == "__main__":
    main(opt["--output_dir"], opt["--clean_df_file_path"], opt["--preprocessed_df_file_path"])